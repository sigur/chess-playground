...
class MappedRouteRouteActionTest {

  @Test
  void executeNoCapture() {
    final RouteStrategy links = new MockedRouteStrategy();
    ...
    final MappedRouteAction move = new MappedRouteAction(links, movement);

    final RouteActionOutcome actual = move.execute(new Coordinate(2, 2), new Coordinate(3, 3));
  ...
  @Test
  void noCapture() {
    final MockedRouteStrategy capture = new MockedRouteStrategy();
  ...
  void pawnEnPassant() {
    ...
    final MappedRouteAction enPassant =
        new MappedRouteAction(
                new DirectionalDeltaRouteStrategy(5, 5, new Coordinate(1, 1), Direction.ASCENT), new EnPassantRouteStrategy(5, 5, new Coordinate(0, 1), Direction.DESCENT)
        );

    final RouteActionOutcome actual = enPassant.execute(new Coordinate(2, 2), new Coordinate(3, 3));
    assertThat(actual.isValid()).isTrue();
    assertThat(actual.getPosition()).hasValue(new Coordinate(3, 3));
